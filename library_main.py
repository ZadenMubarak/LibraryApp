...
def saver():
    ...
    def save():
            ...
            isbn_number = isb.get()
            print(isbn_number)
            author_name = ainfo.get()
            cur.execute(f"INSERT INTO bookTable VALUES ('{book_name.upper()}', '{author_name.upper()}', '{isbn_number}')")
            conm.commit()
            ISBN.delete(0, "end")
            etry.delete(0, "end")
            author.delete(0, "end")
